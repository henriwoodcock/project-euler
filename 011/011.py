...
def vertical_check(matrix):
    ...
    matrix_size=20
    # next check vertical
    # i is going to be matrix column
    for i in range(matrix_size):
        # j is going to be matrix row
...
def diagonal_check(matrix):
    ...
    for j in range(matrix_size-4):
        for i in range(matrix_size-4):
            product=matrix[j][i]*matrix[j+1][i+1]*matrix[j+2][j+2]*matrix[j+3][i+3]
            if product > largest_product:
                largest_product=product
    # now the other diagonal
    for j in range(matrix_size-4):
        for i in range(4, matrix_size):
...
def make_matrix(matrix_string):
    matrix_string = matrix_string.split(' ')
    matrix_size = 20
    matrix = []
    matrix_row = []
    for i in range(len(matrix_string)):
        matrix_row.append(int(matrix_string[i]))
...
if __name__ == '__main__':
    ...
    matrix = make_matrix(matrix_string)
    largest_product=max([horizontal_check(matrix),vertical_check(matrix), diagonal_check(matrix)])
