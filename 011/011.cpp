...
void create_list(int string_length, int skip_amount, char string[],
                int list_of_numbers[]) {
  char str_num[2];
  int num;
  int k=0;
  // create a list of numbers by taking 3 chars at a time
  for (int i=0; i<string_length; i+=skip_amount) {
    // numbers come in "xy " so take [0] = x and [1] = y giving str_num = "xy"
    str_num[0] = string[i];
    str_num[1] = string[i+1];
    // convert to ints and multiply first by 10
    num = (str_num[0] - '0') * 10 + (str_num[1] - '0');
    // add the int to the list of numbers array
    list_of_numbers[k] = num;
    k++;
  }
}
...
int main(void) {
  ...
  const int matrix_length = 20*20;
  int row=0;
  int col=0;
  long horizontal_max;
  int num;
  int list_of_numbers[matrix_size*matrix_size];
  int matrix[matrix_size][matrix_size];
  long long largest_product = 0;
  long long product;

  //update list of numbers to be a list of integers converted from the array
  create_list(sizeof(str)-1, 3, str, list_of_numbers);
  //update matrix to be a matrix_size by matrix_size array of the ints in order
